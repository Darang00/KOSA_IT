				...
				function() {
					...
					$.ajax({
						...
						success : function(data) {
							console.log(data);
							$('#newView').append(data); //댓글 목록에 추가(append)
						},
			...
			<center>
				...
				<table width="80%" border="1">
					...
					<tr>
						...
						<td width="20%" align="center" id="lovecount" ><b>${love.lovecount}</b></td>
					</tr>
					<tr>
					...
					<tr>
						<td colspan="4" align="center"><a
							href="QnABoard.board?cp=${cpage}&ps=${pagesize}">목록가기</a> 
							| <a href="QnABoardEdit.board?idx=${idx}&cp=${cpage}&ps=${pagesize}">편집</a>
							| <a href="QnABoardDelete.board?idx=${idx}&cp=${cpage}&ps=${pagesize}">삭제</a>
							| <button type="button" id="lovecheck">추천</button></td>
					</tr>
				</table>
				<!--  꼬리글 달기 테이블 -->
					...
					<tr>
						<td align="left">작성자 : <input type="text" id="reply_writer"
							name="reply_writer"><br /> 내&nbsp;&nbsp;용 : <textarea
								id="reply_content" name="reply_content" rows="2" cols="50"
						...
						<c:forEach var="reply" items="${replyList}">
							<table width="80%" border="1">
								...
								<tr align="left">
									...
									<td width="20%"><input type="hidden" id="no" name="no"
										value="${reply.no}"> <input type="hidden" id="delidx"
										name="idx" value="${idx}"> password :<input
										type="password" id="delPwd" name="delPwd" size="4"> <!-- <input type="button" value="삭제" id = "reply_del"> -->
										<button type="button" id="reply_del"
											class="btn btn-info rounded-pill m-2">삭제</button></td>
								</tr>
							</table>
						</c:forEach>
					</c:if>
