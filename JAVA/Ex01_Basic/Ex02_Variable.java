...
/*
 블럭주석 : 해석되지 않아요
  ...
  클래스의 종류
  1. class Car { }  >> 독자적인 실행이 불가능 >> 다른 클래스 (설계도) 참조하는 클래스 >> main 함수 (X) >> Library
  2. class Car { public static void main (String[] arg) {}} >> 독자적으로 실행가능한 class
  ...
  공간(크기) : 데이터의 타입(자료형) >> 메모리의 공간은 한정되어 있다 >> 최소한의 공간 최대한 데이터
  ...
  1. instance variable : 객체변수              >> class Car { int number=200; }
...
class Car{ //설계도를 사용하기 위해서는 구체화 ...메모리 위에다가 만드는 행위를 통해서 (연산자 new)
	...
	int window; 
	int speed;  //instance variable 은 초기화를 하지 않아도 된다.
	
	              //내부적으로 기본값(default) : 0 >> int window = 0인 것이죠
	/*
	초기화 : 변수가 처음 값을 갖는 것(할당assign을 통해서)
	질문 : why int window 이런 객체변수는 초기화를 하지 않아도 문제가 생기지 않아요
	자동차를 만들 때 int window=4개 >> 창문 4개...
	...
	public void start() { //강원도
		int speed=100; //지역변수 local variable
	}
}
...
public class Ex02_Variable {
	...
	public static void main(String[] args) {
		...
		//main() { 함수 안에 있는 변수는 지역변수 라 하는데, 이 지역변수는 초기화를 하지 않으면 사용 불가}
		...
		int f2 = f;  //할당
		f=2000; // f변수의 값을 2000 변경시... f2 값은 영향이 없다.
